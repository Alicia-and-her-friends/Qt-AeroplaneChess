...
/*
    我需要在ask类里面创造一个函数，
    使得它可以根据：颜色，所在位置，方向，给出下一个点应该是在什么位置。
...
/*
    update:
    我意识到我的打表功能有些牛逼，我现在甚至可以不需要planelist了。
*/

/*
    begin:
int plane_list::move_func(int dice_num){
        //这个函数的返回值是最终移动到的格子的坐标。

    //当前格点的信息
    square square_now=chatGPT.ask_for_info(pos_now);
    int dire=direction;
        //根据不同格子的类型决定不同的移动方式；
}
    :end
